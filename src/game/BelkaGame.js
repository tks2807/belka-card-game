...
class BelkaGame {
    ...
    async makeMove(playerId, cardIndex) {
        ...
        if (this.state.tableCards.length > 0) {
            ...
            if (isFirstCardTrump) {
                ...
                const hasTrump = player.cards.some(c => c.suit === this.state.trump || c.rank === 'J');
    ...
    async finishRound() {
        ...
        if (this.state.teams.team1.score === 60 && this.state.teams.team2.score === 60) {
            ...
            for (const player of this.state.players) {
                await this.statsService.updatePlayerStats(player.id, player.username, false, // не победа
                0, // очки не важны
                0, // взятки не важны
                true, // isEggs = true
                false, // isGolden
    ...
    async endGame(isGolden, winningTeam) {
    ...
    initializeEndVoting(initiatorId) {
    ...
    checkForReshuffle(playerId) {
        ...
        const suitCounts = player.cards.reduce((counts, card) => {
            if (card.rank !== 'J') {
     ...
     */
    swapPlayers(firstPlayerData, secondPlayerData, chatId) {
        if (!this.state.isActive) {
            return { success: false, message: 'Игра неактивна' };
        }
        if (this.state.tableCards.length > 0) {
