...
class StatsService {
    ...
    async getPlayerELO(playerId) {
        ...
        try {
            const result = await client.query('SELECT elo_rating FROM global_stats WHERE player_id = $1', [playerId]);
    ...
    async getPlayerChatELO(playerId, chatId) {
        const client = await db_1.default.connect();
        try {
            const result = await client.query('SELECT elo_rating FROM chat_stats WHERE player_id = $1 AND chat_id = $2', [playerId, chatId]);
            return result.rows.length > 0 ? result.rows[0].elo_rating : this.RATING_CONSTANTS.BASE_ELO;
        }
        catch (error) {
    ...
    async updateHybridRating(playerId, username, won, score, tricks, isGolden, teammateELO, opponent1ELO, opponent2ELO, chatId) {
        ...
        try {
            ...
            const performanceModifier = this.calculatePerformanceModifier(score, tricks, isGolden, avgScore, avgTricks);
    ...
    async updatePlayerStats(playerId, username, won, score, tricks, eggs, golaya, chatId, countGame = true, rounds = 0) {
        const client = await db_1.default.connect();
